...
class MockQThread(MockQObject):
    ...
    finished = MockSignal()
    def start(self):
        print("[MockQt] QThread.start() - Spawning background thread")
    ...
    def run(self): pass
    def wait(self):
        if hasattr(self, '_t'): self._t.join()
...
class MockQApplication:
    _instance = None
    def __init__(self, args):
        print("[MockQt] QApplication initialized")
        MockQApplication._instance = self
        self.should_quit = False

    def processEvents(self):
        # This is the key verification: If this prints, the UI is responsive.
        # print("[MockQt] processEvents() called")
        pass

    def exec(self):
        print("[MockQt] app.exec() called - Event Loop Started")
        start_time = time.time()
        # Run for at most 6 seconds or until should_quit
        while not self.should_quit and (time.time() - start_time < 6):
            self.processEvents()
            print(".", end="", flush=True) # visual indicator of life
            time.sleep(0.1)
        print("\n[MockQt] app.exec() returning")
        return 0
...
def deploy_assets():
    print("[TEST] deploy_assets called - START (Thread)")
    print("[TEST] simulating heavy work (sleep 3s)...")
    time.sleep(3)
    print("[TEST] deploy_assets - END (Thread)")

class StartupWorker(pyside_core.QThread):
    def run(self):
        deploy_assets()
...
def main():
    ...
    # The FIX: Use StartupWorker
    worker = StartupWorker()

    # We need to keep a reference to win so it doesn't get GC'd (in real app)
    # In this mock, we just define the slot.

    def on_complete():
        print("[TEST] Startup complete signal received!")
        # Post-load
        try:
            ...
            # Tell mock app to quit the loop test
            app.quit()
