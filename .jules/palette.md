...
## 2026-05-27 - QListWidget Multi-selection
**Learning:** By default, PySide6's `QListWidget` only allows single item selection. When implementing list widgets that support batch processing for backend operations (e.g., processing arrays of `selectedItems()`), users expect to be able to select multiple items using modifiers like Shift or Ctrl.
**Action:** Explicitly enable multi-selection via `setSelectionMode(QAbstractItemView.ExtendedSelection)` so users can use Shift/Ctrl modifiers, and communicate this functionality through widget tooltips.
